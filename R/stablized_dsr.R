stablized_dsr=function(dataFrame=NULL, subjectId=NULL, timeToEvent=NULL, measureTime=NULL, covariate=NULL, censoringIndicator=NULL, tau0=6, tradeoff=0.75, method=c("osf", "sm", "pc_glm"), methodFitCensoring="km"){
  ...
  if (method=="osf"){
    ...
    for(i in 1:length(times)){
      sub_data = dataFrame[dataFrame$measureTime==times[i],]
      glm_c[[i]] = glm(indicator~covariate, data=sub_data,weight=1/ipc,family="binomial")
    }

    response=NA
    for(i in 1:nrow(dataFrame)){
      Id=which(dataFrame$measureTime[i]==times)
      response[i]= predict(glm_c[[Id]], newdata=dataFrame$covariate, type="response")
    }
    ...
    model=paste0("geeglm(ifelse(weight_glm>0, 1, 0)~", paste(colnames(dataFrame$covariate), collapse=" + "),", id=sub.id, corstr='independence', data=pc_data,family='binomial')" )
