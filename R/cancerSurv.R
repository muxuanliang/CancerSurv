cancerSurv=function(subjectId=NULL, timeToEvent=NULL, measureTime=NULL, covariate=NULL, censoringIndicator=NULL, tau0=6, tradeoff=0.75, method=c("osf", "sm", "pc_glm"), methodFitCensoring="km"){
  numberCovariate <- NCOL(covariate)
  if (is.null(colnames(covariate))){
    colnames(covariate) <- paste0("v", 1:(NCOL(covariate)))
  }
  dataFrame = data.frame(covariate, subjectId=subjectId, timeToEvent=timeToEvent, measureTime=measureTime, censoringIndicator=censoringIndicator)
  ...
  if (method=="osf"){
    ...
    model=paste0("geeglm(ifelse(weight_glm>0, 1, 0)~", paste(colnames(covariate), collapse=" + "),", id=subjectId, corstr='independence', data=dataFrame,family='binomial')" )
    ...
    for(i in 1:length(times)){
      ...
      sub_data$ipcw[which(sub_data$indicator==0)]=survest(tau0+times[i])
      idx=NA
      for(k in 1:nrow(sub_data)){
